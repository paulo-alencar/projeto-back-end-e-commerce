...
describe("User API", () => {
  ...
  it("should return 400 when creating a user with a duplicate email", async () => {
    const res = await request(app).post("/v1/usuario").send(testUser); // Enviando os mesmos dados de novo
  ...
  it("should get user information by ID and return 200", async () => {
    ...
    const userListRes = await request(app)
      .get("/v1/usuario")
      .set("Authorization", `Bearer ${loginRes.body.token}`); // Supondo que exista uma rota para listar usuários
    ...
    const res = await request(app).get(`/v1/usuario/${userId}`);
